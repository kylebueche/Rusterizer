...
impl Mesh {
    ...
    pub fn uv_sphere(radius: f64, uSegments: usize, vSegments: usize) -> Self {
        ...
        for u in 0..uSegments {
            for v in 0..vSegments {
                ...
                let u_01 = (u as f64) / (uSegments as f64);
                let v_01 = (v as f64) / (vSegments as f64);
                ...
                let wrapped_u = u % (uSegments - 1);
                let wrapped_v = v % (vSegments - 1);
                let top_left = u * vSegments + v;
                let top_right = (u + 1) * vSegments + v;
                let bottom_left = u * vSegments + (v + 1);
                let bottom_right = (u + 1) * vSegments + (v + 1);
        ...
        match draw_mode {
            ...
            DrawMode::Lines => {
                for i in (0..self.indices.len()).step_by(3) {
                    ...
                    let index2 = i + 1;
                    let index3 = i + 2;
                    let point1 = self.vertices[i].scaled_non_uniform(self.scale).rotated(self.rotation).translated(self.position);
                    let point2 = self.vertices[i].scaled_non_uniform(self.scale).rotated(self.rotation).translated(self.position);
                    let point3 = self.vertices[i].scaled_non_uniform(self.scale).rotated(self.rotation).translated(self.position);
